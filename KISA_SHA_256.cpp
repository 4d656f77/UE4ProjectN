...
void SHA256_Process(OUT SHA256_INFO* Info, IN const BYTE* pszMessage, IN UINT uDataLen)
{
	...
	while ((uDataLen + remain_buffer) >= SHA256_DIGEST_BLOCKLEN)
	{
		...
		SHA256_Transform((ULONG*)Info->szBuffer, reinterpret_cast<ULONG*>(Info->uChainVar));
...
// 해쉬 변환 후 문자열 반환
std::string SHA256Encrpyt(IN const BYTE* pszMessage, IN UINT uPlainTextLen)
{
	BYTE out[32] = { 0 };
	SHA256_Encrpyt(pszMessage, uPlainTextLen, out);
	std::stringstream ss;
	ss << std::hex;
	for (int i = 0; i < 32; ++i)
	{
